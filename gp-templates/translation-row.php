				...
				<dd>
					...
					<?php if ( $t->translation_status ): ?>
						<?php if ( $can_approve_translation ) : ?>
							...
							<button class="approve" tabindex="-1" data-nonce="<?php echo esc_attr( wp_create_nonce( 'update-translation-status-current_' . $t->id ) ); ?>" title="<?php echo esc_attr( __( 'Approve this translation.  Any existing translation will be kept as part of the translation history.', 'glotpress' ) ); ?>"><strong>+</strong> <?php _e( 'Approve', 'glotpress' ); ?></button>
							<?php endif; ?>
							<?php if ( 'rejected' !== $t->translation_status ) : ?>
							<button class="reject" tabindex="-1" data-nonce="<?php echo esc_attr( wp_create_nonce( 'update-translation-status-rejected_' . $t->id ) ); ?>" title="<?php echo esc_attr( __( 'Reject this translation.  The existing translation will be kept as part of the translation history.', 'glotpress' ) ); ?>"><strong>&minus;</strong> <?php _e( 'Reject', 'glotpress' ); ?></button>
							<?php endif; ?>
							<?php if ( 'fuzzy' !== $t->translation_status ) : ?>
							<button class="fuzzy" tabindex="-1" data-nonce="<?php echo esc_attr( wp_create_nonce( 'update-translation-status-fuzzy_' . $t->id ) ); ?>" title="<?php echo esc_attr( __( 'Mark this translation as fuzzy for further review.', 'glotpress' ) ); ?>"><strong>~</strong> <?php _e( 'Fuzzy', 'glotpress' ); ?></button>
							<?php endif; ?>
